...
self.isScrolling=!0;self.pid=$ush.timeout(()=>{self.isScrolling=!1},100)}},resize:function(){const self=this;self.winHeight=$ush.parseInt($us.$window.height());self.winWidth=$ush.parseInt($us.$window.width());$us.$body.toggleClass('disable_effects',(self.winWidth<self.options.disableEffectsWidth));if(self.winWidth>self.options.laptopsBreakpoint){$us.$body.usMod('state','default')}else if(self.winWidth>self.options.tabletsBreakpoint){$us.$body.usMod('state','laptops')}else if(self.winWidth>self.options.mobilesBreakpoint){$us.$body.usMod('state','tablets')}else{$us.$body.usMod('state','mobiles')}
if($us.$body.hasClass('us_iframe')){var $frameContent=$('.l-popup-box-content',_window.parent.document),outerHeight=$us.$body.outerHeight(!0);if(outerHeight>0&&$(_window.parent).height()>outerHeight){$frameContent.css('height',outerHeight)}else{$frameContent.css('height','')}}
...
if(!self.hideIfEmpty){self.$container.removeClass('hidden')}
...
$ush.timeout(()=>{const data={postListUid:self.uid,listFilterUid:self.listFilterUid,listFiltersApplied:listFiltersApplied,};$us.$document.trigger('usPostList.itemsLoaded',[$items,data])},50);if(self.hideIfEmpty){self.$container.toggleClass('hidden',$items.length===0)}
...
$us.$canvas.trigger('contentChange')},complete:()=>{self.$container.removeClass('loading_items filtering');delete self.xhr;self.scrollToList()}})},usbReloadIsotopeLayout:function(){const self=this;if(self.$container.hasClass('with_isotope')){self.$list.isotope('layout')}},});$.extend(usPostList.prototype,{initListResultCounter:function(){const self=this;const listResultCounterOpts=[];const $firstList=$(`
...
$message.remove();if(e.type==='checkout_error'){$('.woocommerce-NoticeGroup-checkout').remove()}},});$.extend(WooCommerce.prototype,{ajaxAddToCart:function(e){const self=this;const $button=$(e.currentTarget);const $form=$button.closest('form.cart');if($form.is('[method=get]')){return}
